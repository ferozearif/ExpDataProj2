...
#subset the vehicles and maryland data
veh.dt <- NEI.merged.dt[grepl("Vehicles", Short.Name) & fips == "24510", sum(Emissions), by=year]
# Plot the coal data
library(ggplot2)
png(filename="plot5.png", width=480, height=480)
ggplot(data = veh.dt, aes(x=year, y=V1)) + ggtitle("Trends in emissions from motor vehicle sources in Baltimore City") + xlab("Year") + ylab("Total Emissions in Tons") + geom_line()
dev.off()